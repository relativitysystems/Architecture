...
| **Dropbox** | **Partially implemented** | Legacy plaintext OAuth (`oauth_tokens`) | Connect/status only — `listFiles()` is orphaned code with no calling route; no working import path exists today. |
