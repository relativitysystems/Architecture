...
| **Google Drive** | **Partially implemented** | Encrypted OAuth (`oauth_connections`/`oauth_credentials`, backlog H2) for a persistent connection (no UI entry point) + a separate Picker-obtained browser-token import flow; hashed, server-stored, single-use state (backlog M1, matching Slack) | One-shot import only (Picker UI); no recurring sync, no webhook. Token storage and OAuth state signing now both match Slack. |
| **Dropbox** | **Partially implemented** | Encrypted OAuth (`oauth_connections`/`oauth_credentials`, backlog H2); hashed, server-stored, single-use state (backlog M1, matching Slack) | Connect/status only — `listFiles()` is orphaned code with no calling route; no working import path exists today. |
...
- **State/CSRF protection**: `oauthStateService`'s hashed, single-use, server-side state pattern — used by all three existing providers (Slack, and Google Drive/Dropbox as of backlog M1).
