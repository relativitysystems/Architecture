...
**Status: implementation started 2026-07-30.** Full design exists — [../architecture/LIVE_EMAIL_LOOKUP.md](../architecture/LIVE_EMAIL_LOOKUP.md), milestones EL1–EL12, the [ADR-010](../decisions/ADR-010-LIVE-TOOL-CALLS-ORCHESTRATED-BY-AIKB.md) live-tool-call shape confirmed, and every open product decision resolved as of 2026-07-30 (see that document's Decision Log). EL1–EL9 are substantially implemented as of 2026-07-31: architecture/contracts, the read-only tool registry, the signed `POST /api/tools/execute` endpoint, real Gmail search/content tools behind a 9-gate authorization chain, AIKB's bounded tool-calling orchestration, the portal's mode selector/consent modal/live-source citations, Slack identity linking, Slack live-email access itself, dedicated citation-security tests, an audit trail (`email_ingestion_events`, extended), and a per-connection call-rate budget (in-memory, matching this platform's existing rate-limiter posture) — a live email question now produces a real answer end-to-end from either the portal or a linked Slack DM. Two fields are a real, flagged deferral rather than built half-working: `live_email_attachment` (EL8, needs new Gmail-side attachment metadata parsing) and EL9's `token_usage`/`used_in_final_answer` (an architectural mismatch — the audit row is written before AIKB's later generation call whose usage it would record). EL10 onward are not built. Real-account validation (EL10) remains blocked on EM10.5, unchanged from before this update — EL4–EL9 themselves were built and tested against DI-faked fixtures, not blocked by it.
...
**Status: no longer has a Gmail reference implementation to generalize — needs re-scoping (as of EM10.6, 2026-08-26).** See [CONNECTOR_ROADMAP.md](CONNECTOR_ROADMAP.md)'s Automatic Sync Convergence section for the full per-connector gap table (also flagged there). Every connector should converge toward: background synchronization, version tracking, change detection, retry handling, health monitoring, and user notifications — "connect once, stay updated automatically." This target is unchanged in principle; what's gone is the claim that Gmail already proved a working mechanism to generalize from.

**Why fourth (historical framing, now stale)**: this priority originally read "Gmail already proved this mechanism works (EM8, [ADR-009](../decisions/ADR-009-EMAIL-AUTOMATIC-SYNC-SYSTEM-CLOCK.md)); this priority is about generalizing a proven pattern, not inventing a new one." Two things changed that: (1) EM10.5's staging pass discovered Gmail's automatic-sync cron tick had never actually fired in production at all — "proven" was never true; (2) Automatic Email Ingestion was removed from V1 entirely (EM10.6, [EMAIL_INGESTION.md](../architecture/EMAIL_INGESTION.md)'s EM10.6 record; [ADR-009](../decisions/ADR-009-EMAIL-AUTOMATIC-SYNC-SYSTEM-CLOCK.md) is now marked Superseded). This priority's connector-completeness/connector-health unblocking value for Knowledge Coverage/Analytics may still hold, but the "generalize a proven pattern" framing needs a fresh decision from whoever owns this roadmap next — not resolved here.

**Recommended near-term scope (needs revisiting)**: the original recommendation — a unified connector-health surface generalizing Gmail's existing sync-run view; alerting/notification on sync failure or auth expiry for Gmail and Slack; a deliberate decision on whether Google Drive gets a real recurring-sync engine in Version 1 or explicitly waits — still describes reasonable connector-health work, but "generalizing Gmail's existing sync-run view" no longer has an automatic-sync run to generalize from (manual-sync runs still exist and are still viewable).

**Depends on**: ~~[EM10.5](../architecture/EMAIL_INGESTION.md#em105--gmail-staging-validation) validating Gmail's existing automatic sync against a real account, since that's the pattern being generalized.~~ **Moot** — EM10.5's Scenario 7 (the automatic-sync validation this depended on) is marked N/A/Removed; there is no automatic-sync pattern left to validate or generalize.
...
- ~~Priority 4 (Automatic Sync Polish) also depends on EM10.5, since it generalizes the pattern EM10.5 validates.~~ **Stale as of EM10.6** — see Priority 4's own entry above; there is no longer a Gmail automatic-sync pattern to generalize.
