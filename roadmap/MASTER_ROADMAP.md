...
6. **Then proceed with the next major connector or Knowledge Sync work** (see [Later Product Expansion](#later-product-expansion)), and continue the remaining Track A engineering items below in parallel throughout:
   - 🔶 **Mostly closed (backlog H4).** The shared-`x-api-key`-only tenant-authorization gap on AIKB's non-Slack management routes is closed for 10 of 14 routes (`/ingest`, `/reindex`, document/client delete, documents/collections listing, collections CRUD) via the same signed HMAC envelope `/ask` already used. `/jobs`, `/summary`, `/analytics`, `/stats` remain shared-key-only — tracked as H4's residual scope. See [../decisions/ADR-004-SIGNED-SERVICE-REQUESTS.md](../decisions/ADR-004-SIGNED-SERVICE-REQUESTS.md).
