...
**Status: implementation started 2026-07-30.** Full design exists — [../architecture/LIVE_EMAIL_LOOKUP.md](../architecture/LIVE_EMAIL_LOOKUP.md), milestones EL1–EL12, the [ADR-010](../decisions/ADR-010-LIVE-TOOL-CALLS-ORCHESTRATED-BY-AIKB.md) live-tool-call shape confirmed, and every open product decision resolved as of 2026-07-30 (see that document's Decision Log). EL1–EL3 are implemented: architecture/contracts, the read-only tool registry (`aikb/services/emailToolSchemas.js`, `Relativity/services/emailToolValidation.js`), and the signed `POST /api/tools/execute` endpoint proven end-to-end with one hardcoded no-op tool (`Relativity/routes/toolExecution.js`, `aikb/services/toolExecutionClient.js`). EL4 onward (real Gmail calls, mailbox authorization, AIKB orchestration, portal/Slack UI) are not.
