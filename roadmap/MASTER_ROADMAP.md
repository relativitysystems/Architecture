...
- Any future connector (Teams, Outlook) should wait until the shared-`x-api-key` gap is closed or explicitly accepted as a known risk for that connector too, since each new connector otherwise inherits the same weak service-to-service trust model Slack's `/ask` path deliberately avoided. See [../decisions/ADR-004-SIGNED-SERVICE-REQUESTS.md](../decisions/ADR-004-SIGNED-SERVICE-REQUESTS.md). **Gmail is an exception, already built**: its Relativity→AIKB calls (`services/aikbService.js`'s `uploadAndIngest`/`listDocuments`/`deleteDocumentById`/etc.) already use ADR-004's signed-envelope pattern (`signedEnvelope()`, HMAC-bound to `clientId`), the same mechanism Slack's `/ask` path uses — this dependency was satisfied by construction for Gmail, not explicitly re-decided as a project gate before EM1 started; this bullet's guidance was not retroactively verified as followed, only found to hold true on inspection.
...
- **Gmail is no longer "not scheduled" — it has been substantially built.** Per its full design ([../architecture/EMAIL_INGESTION.md](../architecture/EMAIL_INGESTION.md)), EM1 through EM10 are implemented (2026-07-23 through 2026-07-25): per-member OAuth connect, the organization policy engine, the Gmail-label consent workflow, historical + incremental sync into the shared AIKB pipeline, an automatic-sync scheduler, member offboarding/policy-change reconciliation, and real citations with subject/sender/date/deep links. **Not yet done**: real-account validation (no milestone through EM10 has been exercised against a live Gmail account — a dedicated milestone, EM10.5, exists specifically to close that gap), attachments (EM11, blocked on an unresolved malware-scanning decision), and Outlook (EM12, deliberately sequenced after EM10.5).
- Additional connectors beyond Gmail: Microsoft Teams, Outlook, meeting-transcript sources, CRM systems — all following the pattern Slack established ([../architecture/CONNECTOR_FRAMEWORK.md](../architecture/CONNECTOR_FRAMEWORK.md)) and, for Outlook specifically, the per-member/label-consent pattern Gmail has now proven. None of these has an adapter built yet.
