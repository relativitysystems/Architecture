...
- **Shared `x-api-key` grants broad trust — narrowed, not eliminated (backlog H4).** `/ingest`, `/reindex`, document delete, client delete, documents/collections listing, and collections CRUD (10 routes) now additionally require the signed HMAC envelope already used by `/ask` (`requireServiceRequest`), which cryptographically binds `clientId` to the request — a leaked shared key alone is no longer sufficient on these routes. `/jobs`, `/summary`, `/analytics`, and `/stats` (4 routes) still trust the shared key plus whatever `clientId` the caller supplies alone, relying on Relativity to have already verified entitlement upstream — tracked as H4's remaining scope.
