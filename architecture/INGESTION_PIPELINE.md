...
- **Writer**: Relativity uploads the raw bytes directly to AIKB's Storage bucket (using AIKB's own service-role credentials configured in Relativity's environment) — this is a direct cross-service Storage write, not proxied through an AIKB API route. This write always happens *before* AIKB's own content-hash/status dedup check runs — Relativity has no way to know in advance whether AIKB will skip, rebuild, or freshly index the result.
...
- **Overwrite policy (EM10.5 Bug #8)**: whether the write may overwrite an object already at that path is decided per `sourceProvider`, in one place — `Relativity/services/aikbService.js`'s `allowsStorageOverwrite()`. Portal upload, ZIP import, and Google Drive import mint a fresh `crypto.randomUUID()` `sourceFileId` per call, so the path can never legitimately pre-exist there; they keep `upsert: false`, which fails loudly on a collision rather than silently overwriting unrelated content. Gmail ingestion passes the *stable* Gmail message id as `sourceFileId`, which is legitimately re-uploaded on every re-scan (Full Scan revisits every currently-matching message unconditionally, with no local "already ingested" pre-filter) — it uses `upsert: true`, a direct atomic overwrite at the same path, so a repeat scan or a delete/re-import cycle reaches AIKB instead of failing at the storage write. `microsoft` is not yet a real ingestion path in Relativity and is excluded from the overwrite allowlist until one exists.
