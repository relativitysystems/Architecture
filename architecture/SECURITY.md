...
- **Owner invite / team invite flows**: owner invites embed `client_id` in Supabase user metadata at creation time, read server-side only (never trusted from the browser) on completion. Team-member invite tokens are 32 random bytes, hex-encoded, with a 7-day expiry and single-use semantics enforced by status-transition guards. ~~The token itself is stored **in plaintext** in `team_invites.token`~~ — **hash-only storage implemented (backlog M2), same pattern as OAuth state below.** `team_invites.token_hash` (SHA-256 hex) replaces the plaintext column; the raw token is still what's generated and emailed, only its at-rest storage changed. **Code-complete but the migration (`Relativity/supabase/migrations/20260719_hash_team_invite_tokens.sql`) has not yet been applied to the live database** — this environment's Supabase MCP access is read-only by configuration; an operator needs to run it before this code path deploys. See [FEATURE_BACKLOG.md](../roadmap/FEATURE_BACKLOG.md) M2.
