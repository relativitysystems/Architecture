...
All secrets are read from environment variables only; no secrets-manager or vault integration exists in either repository. Notable variables include (Relativity): `ADMIN_PASSWORD`, `ADMIN_JWT_SECRET`, `GLOBAL_SUPABASE_SERVICE_ROLE_KEY`, `AIKB_API_KEY`, `SLACK_CLIENT_SECRET`/`SLACK_SIGNING_SECRET`, `INTEGRATION_CREDENTIAL_ENCRYPTION_KEY` (with a deprecated fallback to the older `SLACK_TOKEN_ENCRYPTION_KEY`), `SERVICE_REQUEST_SIGNING_SECRET`; and (AIKB): `AIKB_SUPABASE_SERVICE_KEY`, `GLOBAL_SUPABASE_SERVICE_KEY`, `OPENAI_API_KEY`, `INNGEST_SIGNING_KEY`, `API_KEY`, `SLACK_SIGNING_SECRET`. `CRON_SECRET` was removed from Relativity's `.env.example` along with the sweep endpoint it protected ([ADR-007](../decisions/ADR-007-SLACK-BOUNDED-DELIVERY-RETRY.md)); `DROPBOX_APP_SECRET`/`GOOGLE_CLIENT_SECRET` were removed the same way when backlog M15 deleted the OAuth connect flows that read them (Google Drive's Picker import needs only `GOOGLE_CLIENT_ID`/`GOOGLE_PICKER_API_KEY`, no secret).

**OAuth credentials at rest (Slack, since Milestone 3; briefly also Google Drive and Dropbox — backlog H2, reverted by M15)** are encrypted with AES-256-GCM in `Relativity/services/integrationCredentialEncryption.js`:
...
**Slack is the only provider this repo still writes here.** Google Drive and Dropbox were briefly migrated onto the same `oauth_connections`/`oauth_credentials` model Slack uses (backlog H2), then had their entire persistent-connection flow removed outright (backlog M15) — not reverted back to plaintext storage, deleted entirely, since neither had a working use for the connection. The legacy plaintext `oauth_tokens` table is no longer written to by anything; `upsertToken`/`getToken` remain in `supabaseService.js` only because the table itself hasn't been dropped, not because anything still calls them.
...
- **OAuth state (CSRF protection on connect flows)**: Slack's flow stores only a SHA-256 hash of a 32-byte random state value, with a 10-minute TTL and atomic single-use consumption. ~~Google Drive's and Dropbox's OAuth `state` parameters remain unsigned base64-encoded JSON~~ — briefly brought to the same hashed pattern (backlog M1), then made moot entirely: both providers' `/start`/`callback` routes were deleted outright (backlog M15), along with the persistent connection they protected. `oauthStateService` remains provider-generic and available to any future connector. See [FEATURE_BACKLOG.md](../roadmap/FEATURE_BACKLOG.md) M1, M15.
...
5. ~~**Unsigned OAuth state for Google Drive and Dropbox**~~ — resolved by M1, then made moot entirely: **removed (backlog M15)** along with the rest of both providers' persistent-connection flow.
...
8. ~~**Google Drive and Dropbox OAuth tokens remain plaintext at rest**~~ — briefly resolved (backlog H2) by writing through the same encrypted `oauth_connections`/`oauth_credentials` model as Slack, then made moot entirely: **removed (backlog M15)**, both providers' persistent-connection flow deleted outright rather than kept encrypted-but-unused. See [ADR-006](../decisions/ADR-006-OAUTH-CREDENTIAL-ENCRYPTION.md).
...
- ~~Extend AES-256-GCM encrypted storage (`oauth_connections`/`oauth_credentials`) to Google Drive and Dropbox~~ — done, backlog H2, then reverted by removing both providers' connection flow entirely (backlog M15) rather than leaving encrypted-but-unused credentials in place. `oauth_tokens` itself has not been dropped (kept as a defensive landing spot; confirmed empty for every provider as of this writing).
