...
| Service-request HMAC envelope | `services/serviceRequestAuth.js` (implemented identically in both repos) | The highest-trust machine-to-machine calls: AIKB's `POST /ask` and `POST /chat/redact` (Relativity → AIKB) and Relativity's `POST /api/integrations/slack/deliver` (AIKB → Relativity, reversed direction). Signature = `HMAC-SHA256(secret, "requestId.issuedAt.expiresAt.clientId.idempotencyKey.sha256(payload)")`, 60-second TTL, `crypto.timingSafeEqual`. `clientId` is read only from the verified envelope, never from the raw request body, in every direction. `POST /chat/redact` (ADR-007, implemented) reuses this exact mechanism to let Relativity tell AIKB which chat session to redact by `idempotencyKey` after a Slack event reaches `delivery_failed`. |
...
AIKB's own `x-api-key`-gated routes (ingest/list/delete under `/api/knowledge`) remain a separate, still-open item: they are deliberately documented as relying on Relativity (the sole intended caller) to have already enforced per-client entitlement, and additionally perform their own defense-in-depth ownership check (`doc.client_id !== clientId → 403`) before acting — this is a documented trust assumption, not an unrecognized gap, provided Relativity itself validates entitlement upstream. See [FEATURE_BACKLOG.md](../roadmap/FEATURE_BACKLOG.md) H4.
