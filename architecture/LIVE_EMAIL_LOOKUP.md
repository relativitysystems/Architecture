...
**Status of this document: Proposed, with implementation underway. EL1–EL9 are substantially implemented as of 2026-07-31** — architecture/contracts (EL1), the read-only tool registry (EL2, [§EL2](#el2--read-only-tool-registry)), the signed AIKB→Relativity execution endpoint (EL3, [§EL3](#el3--signed-aikb-to-relativity-execution-endpoint)), real Gmail search/content tools behind the 9-gate authorization chain (EL4, [§EL4](#el4--gmail-search-and-content-tools)), AIKB's bounded tool-calling orchestration (EL5, [§EL5](#el5--aikb-bounded-tool-orchestration)), the portal's mode selector/consent modal/live-source citations (EL6, [§EL6](#el6--portal-automaticlive-search-experience)), Slack identity linking (EL7A, [§EL7A](#el7a--slack-identity-linking)), Slack live-email access itself (EL7B, [§EL7B](#el7b--slack-live-email-access)), citation-security hardening (EL8, [§EL8](#el8--citations-and-live-source-ui)), and audit/budget enforcement (EL9, [§EL9](#el9--audit-budgets-and-observability) — the audit write landed with EL7B; this pass added the per-connection call-rate budget and its own dedicated content-safety test; `token_usage`/`used_in_final_answer` are a real, flagged deferral, not built). **EL10 onward remain unimplemented** — EL4–EL9 have not been validated against a real Gmail account (blocked on [EM10.5](EMAIL_INGESTION.md#em105--gmail-staging-validation), not yet run — see that milestone's own status), and no prompt-injection red-team testing has been done. Treat each milestone's own Status line as authoritative over this banner if the two ever disagree.
...
- **Org-wide admin policy** — a new `email_organization_settings.live_lookup_enabled` boolean (default `false`, fail-closed), owner/admin-only, on the same settings panel. *(This originally read "mirrors `automatic_sync_enabled`'s existing default... alongside the existing automatic-sync toggle... independent of `automatic_sync_enabled`" — that ingestion-side toggle and column were dropped in EM10.6 along with Automatic Email Ingestion; `live_lookup_enabled` is now the only setting on this panel, and this whole paragraph's "why these must not be the same flag" reasoning is preserved as design history, not a live distinction.)* Confirmed: this must remain a hard, complete disable — when off, the tools are never offered to the model for any member of that client, regardless of individual mode-selector preference.
...
**Update, confirmed 2026-08-05 — first step of this direction shipped**: the prominent three-way mode selector described in [§2.1](#21-design-mvp) (the `Email search` label plus `Automatic`/`Company knowledge only`/`Live email` dropdown, next to the "Search scope" collections filter) has been **removed from the standard client portal UI**. The portal now silently defaults every query to `automatic` — the same canonical value `routes/api.js` already fell back to for a missing/invalid `emailLookupMode`, so no backend behavior changed. `company_knowledge` and `live_email` remain fully implemented and untouched server-side (`emailLiveLookupService.js`, `routes/api.js`'s `EMAIL_LOOKUP_MODES`/`forceLiveLookup`) and stay available to the automatic router; only the manual selector is gone. This is a UI-only step, not the full §2.2 target shape — the small advanced source-control menu and the explicit "Search live email" shortcut described below are **not yet built**.
...
| | MVP | Later |
|---|---|---|
| Mode selector | ~~✅ Company knowledge / Live email / Automatic, prominent~~ **Removed 2026-08-05** — portal now silently defaults to `automatic`; see the update note in [§2.2](#22-long-term-ux-direction-confirmed-adjustment-2026-07-30) | Still pending: a small advanced source-control menu + an explicit "Search live email" shortcut, [§2.2](#22-long-term-ux-direction-confirmed-adjustment-2026-07-30) |
...
**Model receives a structured schema, never raw Gmail search syntax.** Gmail's query language (`from:`, `after:`, `-in:chats`, etc.) is compiled server-side from the structured arguments by Relativity, reusing `gmailService.js`'s existing `compileSearchQuery` pattern — the model never sees or constructs a raw query string, so there is no way for a crafted argument to smuggle an unintended Gmail search operator through.
...
| "What are the recurring complaints customers have emailed us about this year?" | **Ingestion only** | A pattern-across-many-messages question is exactly what a vector index over ingested email is good at and live lookup is bad at (bounded to ≤25 results per call, no aggregation) — this is the clearest case for steering *toward* ingestion rather than live lookup, including recommending the client label more of this content (see [§11](#11-additional-product-features), "recommend ingestion") |
...
- **This is gated by a separate, explicit toggle instead** (`live_lookup_enabled`, org-wide and per-mailbox, [§2](#2-portal-activation)/[§7](#7-security-and-threat-model)) precisely because it is a materially different trust decision than ingestion's allow-rule policy, and conflating the two flags would either (a) silently expand what ingestion policy implies it's approving, or (b) make live lookup unusable under a conservative, label-only ingestion policy that was never meant to describe live-read boundaries in the first place.
...
- **Status: Implemented (2026-07-31).** See the Implementation Record immediately below. EL6 onward remain unimplemented; real-account validation blocked on EM10.5.
...
- **Schema/env changes**: one additive migration (two new boolean columns, both nullable/defaulted `false` — fail-closed).
...
#### EL4 Implementation Record

`Relativity/services/emailLiveLookupService.js` (new, `createEmailLiveLookupService(deps)` factory) implements the 8-gate authorization chain (`resolveAuthorizedMailbox`) — member status/role/`search_enabled`, an active `oauth_connections` row, its `email_connections` row, org `email_organization_settings.live_lookup_enabled`, per-mailbox `email_connections.live_lookup_enabled`, and a valid/refreshable access token via the existing `emailConnectionService.getValidGmailAccessToken` — plus `searchEmailMessages`/`getEmailContent`. Every Gmail call reuses `gmailService.js`'s existing primitives unchanged (`listMessageIdsByQuery`/`getMessageMetadata`/`getMessageBody`); deny-rule re-verification reuses `emailPolicyService.js#ruleMatchesMessage` directly against deny rules only (§5.3 — no allow-rule gate). One genuinely new Gmail-facing call was needed: `gmailService.js#getThread` (`users.threads.get?format=full`), for `get_email_content`'s `threadId` branch — Gmail's search syntax has no `threadId:` operator, so a single-message loop isn't an option; it reuses the file's existing header/body-parsing helpers unchanged. `getMessageMetadata` also gained a `snippet` field (Gmail returns it on every format except `raw`; previously unread by any caller) to back `search_email_messages`'s result preview without a second fetch. New config (`Relativity/config/index.js`, `.env.example`): `email.liveLookup.{snippetMaxChars, bodyMaxChars, searchTimeoutMs, contentTimeoutMs, defaultMaxHistoricalDays}`. Migration `20260731_email_live_lookup_el4.sql` adds both boolean columns (default `false`) and folds `live_lookup_enabled` into EM9's existing atomic `offboard_client_member` cascade, applied to the Global Supabase project and verified via direct query. `toolExecutionService.js`'s `unknown_tool` branch is replaced with real dispatch to the two tools (validated first via EL2's `emailToolValidation.js`); `routes/toolExecution.js` now passes `clientId` (from the verified envelope) and `requestingMemberId` (from the payload) through. 25 new tests in `Relativity/test/emailLiveLookupService.test.js` covering every gate's rejection case, deny-rule filtering, cap enforcement, cross-member isolation, and both `get_email_content` branches.

**Known, deliberate gap**: `gmailService.js` doesn't propagate Gmail's real HTTP status code out of its `HTTP_ERROR`, so `emailLiveLookupService.js` cannot distinguish a 429 rate limit from any other failed call — both collapse to `provider_timeout`, per §9's own precedent for "the call didn't complete." A `rate_limited` result is not yet reachable.

Full suites green: 765/765 Relativity.

### EL5 — AIKB bounded tool orchestration
- **Status: Implemented (2026-07-31).** See the Implementation Record immediately below. Portal/Slack UI (EL6+) and real-account validation (EM10.5) remain outstanding.
...
#### EL5 Implementation Record

`aikb/services/openaiService.js`: `classifyQueryIntent` gains `mayNeedLiveEmailLookup` (a new classifier prompt section + output field), defaulted `false` on every guardrail path and the error fallback, and coerced (not thrown) to `false` if the model's response omits/malforms it — the two-stage gate's cheap half (§1.3 Option C). `generateRagAnswer` itself is byte-for-byte unchanged; a new sibling pair, `generateRagAnswerWithTools`/`continueRagAnswerWithToolResult`, shares its message-building via an extracted `buildRagMessages` helper and returns `{type:'text', content}` or `{type:'tool_call', conversationMessages, toolCall}` — `conversationMessages` already includes the assistant's `tool_calls` message, ready to pass to the next call per the OpenAI API's required ordering. `RAG_SYSTEM_PROMPT` gained one paragraph instructing the model to treat tool results as untrusted data, never instructions, and to state a `status:"unavailable"/"error"` reason verbatim rather than paraphrasing it as "no matching email found" (§7, §9).

`aikb/services/runKnowledgeQuery.js` gains `emailLookupAvailable` (default `false`) and the orchestration loop (`runEmailToolAugmentedGeneration`). The ADR-010 Conformance two-call bound is enforced **structurally**, not by runtime counting: call 1 is offered only `[SEARCH_EMAIL_MESSAGES_TOOL]`, call 2 (if reached) only `[GET_EMAIL_CONTENT_TOOL]`, and the final call (if reached) no tools at all — the model cannot fetch before searching, search twice, or make a third call, because the schema it's given each time never allows it. A `get_email_content` request for a `messageId`/`threadId` not present in call 1's own results is rejected locally (`{status:'error', reason:'validation_error'}`, fed back to the model) and never sent to Relativity — the "cannot fetch an id it didn't receive from Call 1" requirement. Both tool calls go through the existing `toolExecutionClient.js` (EL3) unchanged, each with a fresh `crypto.randomUUID()` idempotency key. Live sources (`live_email_message`/`live_email_thread`, §6.2, including `deepLinkUrl` — a small EL4 follow-up added this field to `emailLiveLookupService.js`'s results) merge into the same `sources` array as stored-knowledge citations; when at least one live source came back, `isKnowledgeGapAnswer`'s phrase-matching is skipped entirely, so a correct live-email answer can never be misclassified as "not documented in the knowledge base." The zero-stored-chunks immediate-gap shortcut now only fires when the tools gate is also closed — when live lookup is available, generation still runs (possibly with zero stored chunks) so the model gets a chance to answer from a live search alone, per §1.1's "Live email lookup only" sequence.

**Explicitly deferred, not part of this milestone's scope**: §8.3's "deduplication against already-ingested emails" token-control optimization (checking `email_source_messages` for a `providerMessageId` before fetching live) — a cost optimization, not core orchestration, left for a follow-up once real usage data justifies it.

21 new/rewritten tests in `aikb/test/runKnowledgeQuery.test.js` (gate-closed on each of the three independent signals, gate-open-no-call, single-tool-call, two-call search-then-fetch, unknown-id rejection, hybrid source-merging, gap-suppression, provider-failure handling) plus a fix to that file's own pre-existing missing env-var bootstrap (unrelated to this milestone, needed to run these tests at all).

Full suites green except one pre-existing, unrelated failure: 150/151 AIKB (`test/slackEventsRetired.test.js` fails on the same missing-env-var class of issue, not yet fixed there — out of this milestone's scope).

### EL6 — Portal automatic/live-search experience
- **Status: Implemented (2026-07-31).** See the Implementation Record immediately below. Not interactively verified in a browser (no real authenticated Supabase session available in the implementing environment) — verified via the full test suite, `app.js` booting cleanly with the new routes wired, and balanced HTML structure; a real browser walkthrough is still recommended before this ships to users.
...
#### EL6 Implementation Record

**Naming discrepancy resolved**: this milestone's own summary table (above) names the consent column `live_lookup_ack_at`; [§2.3](#23-consent-design-confirmed-2026-07-30) — the more detailed, explicitly dated design section — names it `live_lookup_consented_at` and defines its exact semantics. Implementation follows §2.3 as authoritative; the migration comment documents the discrepancy for anyone who later needs to reconcile it.

**Backend** (Relativity): migration `20260731_email_live_lookup_el6.sql` adds `client_members.live_lookup_consented_at` (nullable timestamp, fail-closed). `emailLiveLookupService.js`'s authorization chain (EL4) gains a 9th gate — consent — re-verified on every request like every other gate there; a real, silent EL4 bug was found and fixed in the same change: `supabaseService.js#getClientMemberById` never selected `search_enabled`, so that gate has silently never fired since EL4 shipped (the column now joins `live_lookup_consented_at` in the same query). A new, deliberately cheap `isLiveLookupAvailable` check (DB reads only, no Gmail token refresh) is what `routes/api.js`'s `POST /knowledge/query` now actually runs on every question to compute the `emailLookupAvailable` flag AIKB receives — this flag was always `false` before EL6 since nothing computed or sent it (EL5's own stated deferral). New `GET`/`PUT /api/integrations/email/live-lookup-settings` (member-scoped: `PUT {consent}` atomically sets both `client_members.live_lookup_consented_at` and the member's own `email_connections.live_lookup_enabled` in the same request, per §2.3's "revocation IS the consent record" design — one user-facing on/off, not two independently-tracked switches); `PUT /settings` extended for the org-wide `live_lookup_enabled` toggle (at the time, optional and independent of `automaticSyncEnabled`; that ingestion-side field was dropped in EM10.6, so `liveLookupEnabled` is `PUT /settings`'s only body field now — see [EMAIL_INGESTION.md](EMAIL_INGESTION.md)'s EM10.6 record). A three-way `emailLookupMode` (`company_knowledge`/`live_email`/`automatic`) on `POST /knowledge/query` computes `forceLiveLookup`, forwarded to AIKB.

**Backend** (AIKB): `routes/knowledge.js`'s `POST /query` and `runKnowledgeQuery.js` accept `forceLiveLookup` — bypasses the classifier's own `mayNeedLiveEmailLookup` signal specifically (the "Live email" mode override, §2.1), never any other gate (`emailLookupAvailable` and every downstream authorization gate still fully apply).

**Frontend** (Relativity portal): a three-way mode selector next to the existing "Search scope" collections filter, `localStorage`-persisted per client (matching `kbAllowedCollectionIds`'s existing precedent, backlog M10). A one-time consent modal (reusing the existing `.team-invite-modal` markup/CSS pattern, no new modal component), triggered when a live-capable mode is active and the member hasn't consented yet. A static "Searching connected email…" hint joins the existing loading-dots bubble (§2.4's documented MVP limitation — `/query` isn't streamed, so there's no mid-request signal for a real tool-call-in-flight state). Live sources (`live_email_message`/`live_email_thread`) render in a visually distinct "Live sources" box with a small "🔴 Live" badge, never merged into the existing "Sources" box (§6.3) — `portalCitations.js` gained `isLiveSource`/`citationDate` (stored sources use `sentAt`, live sources use `receivedAt`), reused by the same thread-grouping logic stored email citations already used.

11 new tests (`emailConnectionService.test.js`, `emailRoutes.test.js`, `runKnowledgeQuery.test.js`, `portalCitations.test.js`). Full suites green: 776/776 Relativity; 150/151 AIKB (the one pre-existing, unrelated failure — `test/slackEventsRetired.test.js`'s own missing-env-var bootstrap gap, noted in EL5's record — remains unfixed there).
...
- **Status: Implemented (2026-07-31).** See the Implementation Record below. No live-lookup behavior consumes this mapping yet — that's EL7B, not started.
...
#### EL7A Implementation Record

**Status: Implemented (2026-07-31).** A dedicated `slack_link_codes` table (migration `20260731_slack_identity_linking_el7a.sql`, applied) rather than literally reusing `oauth_states` — that table's schema (`provider`, `redirect_after`) is OAuth-redirect-flow-specific, not a generic code-payload table; `slack_link_codes` mirrors `oauth_states`' hash-only-storage/atomic-single-use-consumption/TTL pattern exactly instead. `slack_user_links` (`client_id`, `slack_user_id`, `slack_team_id`, `member_id`, `linked_at`, unique on `(client_id, slack_user_id)`) landed as specified.

`Relativity/services/slackUserLinkService.js` (new): `generateLinkCode`, `completeLink`, `getLinkedMember` (EL7B's own resolver, built here since it's identity plumbing, not duplicated into `slackEventsService.js`). Codes are 8 characters from a 32-symbol alphabet excluding visually ambiguous characters (`0`/`O`, `1`/`I`/`L`) — chosen over `oauthStateService`'s 64-hex-character browser-redirect state, since this code is typed by hand into a Slack DM. `completeLink`'s `clientId` cross-check (the Slack workspace's own resolved client, from `resolveWorkspace`, never trusted from the payload) rejects — and burns, no retry — a code submitted under a different client's workspace than the one it was generated for, a case the original spec's "cross-client isolation" test requirement named but didn't fully spell out the mechanism for.

**Slack-side command shape resolved** (left as "implementation detail, not decided here" above): rather than a registered Slack slash command (which needs external Slack App configuration outside either repository), a DM matching `link CODE` (case-insensitive, explicit trigger word required) is intercepted in `Relativity/services/slackEventsService.js#processEventCallback`, after event-log dedup but before `extractQuestion`/the AIKB ask pipeline — a link attempt (valid or not) never reaches AIKB. Reuses the existing bounded-retry reply machinery (`replyDirectly`, ADR-007) unchanged. `Relativity/routes/integrations/slack.js` gained `POST /link/generate-code` (self-service, any active member); the portal's Email panel gained a "Link Slack account" control (shown only while a Gmail connection is active) displaying the generated code and instructions.

34 new tests (`slackUserLinkService.test.js`, `slackEventsService.test.js`, `slackRoutes.test.js`). Full suite green: 798/798 Relativity.

### EL7B — Slack live-email access
- **Status: Implemented (2026-07-31).** See the Implementation Record below.
...
#### EL7B Implementation Record

**The EL9 dependency was real, not aspirational** — EL7B's own acceptance criteria requires an audit row per interaction, and full EL9 (soft token budgets, per-connection rate budgets) doesn't exist. Rather than leave this dependency unmet or build all of EL9, a narrow, focused slice was pulled forward: `email_ingestion_events` (EM1) gained two new outcome values (`live_lookup_search`, `live_lookup_fetch`), a relaxed `provider_message_id` (nullable, since a search has no single message id — a new CHECK keeps it required for every other outcome), and five new nullable columns (`origin`, `origin_metadata`, `tool_name`, `result_count`, `provider_latency_ms`). `services/emailLiveLookupService.js` writes one best-effort row per tool execution that gets past its authorization gate chain (a gate rejection writes nothing — nothing was attempted against Gmail). `token_usage`/`used_in_final_answer` (full EL9 fields, needing AIKB-side wiring this pass doesn't build) are deliberately not included.

`requestingMemberId` resolution (`services/slackEventsService.js`) happens **only** inside the existing `isDirectMessage` branch — structurally unreachable for `app_mention`, not merely gated by an if-check, so the [§3.3](#33-recommended-mvp-policy--confirmed-2026-07-30) "channel `@mentions` never offer the tools regardless of link status" requirement can't regress via a refactor that forgets a condition. `emailLookupAvailable` reuses the exact same cheap, DB-only check the portal uses (EL6's `isLiveLookupAvailable`) — no Gmail call on every single Slack question. Both fields thread through `aikbAskClient.js` → AIKB's `POST /ask` → the `knowledge/slack.question.requested` Inngest event → `runKnowledgeQuery` unchanged from EL5/EL6's own shape, defaulting to `null`/`false` for every pre-EL7B caller.

**The "never a silent failure" requirement resolved without new AIKB-side classifier plumbing**: `runKnowledgeQuery`'s `intent.mayNeedLiveEmailLookup` (EL5) was already computed and already present in its return value — `aikb/inngest/functions.js` now derives one narrow boolean, `emailLookupSuggested` (`mayNeedLiveEmailLookup && !emailLookupAvailable`), and includes only that in the deliver-to-relativity payload, never the full `intent` object. `services/slackDeliverService.js` turns it into a hint appended via `slackAnswerFormatter.js`, deliberately generic wording covering both an unlinked Slack user and a linked one with no active/consented mailbox (Relativity doesn't re-distinguish the two at deliver time).

**A real, pre-existing correctness gap found and fixed along the way**: `slackAnswerFormatter.js#formatCitations` required a source's `title`/`fileName` field to render a line at all — live sources (§6.2) carry `subject`/`from` instead, so a linked user's live-email answer would have shipped with its citations silently dropped in Slack. Fixed with a `titleForSource` fallback (the same `"subject" from sender` construction `runKnowledgeQuery.js`'s own `sourceMap` already uses for a *stored* email citation's `title`), plus a `(Live)` marker mirroring the portal's own badge.
...
- **Deferred**: `live_email_attachment` (see Implementation Record — a real, flagged deferral, not an oversight).

#### EL8 Implementation Record

**What was already done before this pass, and where**: the "Live sources" grouping, sender/date/deep-link display, and "🔴 Live" badge landed in the portal during EL6 (`portal.js#buildLiveSourcesBox`/`renderEmailCitationLine`, `portalCitations.js#isLiveSource`/`citationDate`) — built then because EL6's own acceptance criteria needed a working portal live-lookup experience, not deferred to EL8. Slack citation rendering (`slackAnswerFormatter.js#titleForSource`, the `(Live)` marker) landed in EL7B for the identical reason: a linked Slack user's live-email answer needs its citations to actually appear. Both satisfy this milestone's acceptance criteria (subject/sender/date/deep-link, visually distinct "Live" marking) as originally written.

**This pass's actual work**: the one concrete, not-yet-done piece was the security requirement's own explicit instruction — "a dedicated test, not just code review." Added across every layer a citation passes through: `Relativity/services/emailLiveLookupService.js` (a poisoned Gmail metadata/body response never propagates into a search match, a fetched message, or the EL7B audit row), `aikb/services/runKnowledgeQuery.js` (now exports `liveSourcesFromSearchMatches`/`liveSourcesFromContentMessages` so a poisoned tool-result object can be asserted directly against — including a hybrid stored+live case), `Relativity/services/slackAnswerFormatter.js` (`titleForSource`/`formatCitations`/`formatSlackMessage` against a poisoned source), and `Relativity/public/portal/portalCitations.js` (documents that `isEmailSource`/`isLiveSource`/`citationDate`/`groupSourcesForDisplay` never reshape a source object at all — they return a boolean, a single field, or grouped *references* — which is what makes portal.js's actual DOM-rendering code, verified by code review rather than a jsdom test since no such harness exists in this repo's conventions, safe: it reads only `.subject`/`.from`/`citationDate(s)`/`.deepLinkUrl`).

**`live_email_attachment` is deliberately NOT built, despite this milestone's own "Deferred: nothing significant" framing.** §6.1 describes it as "MVP-usable today since it never depends on attachment content," but that undersells what it actually needs: EL4's `search_email_messages` result shape carries no attachment information at all today — `gmailService.js#getMessageMetadata` only parses `Subject`/`From`/`Date` headers, never attachment presence or filenames, even though `hasAttachment`/`attachmentNameContains` are valid *search filters* ([§4.2](#42-search_email_messages)). Citing "David sent an attachment named X" would need genuinely new Gmail-side metadata parsing (walking `payload.parts[].filename` the same way `extractBodyParts` already walks parts for body content) that was in scope for neither EL4 nor this milestone. Flagged explicitly here rather than silently shipping a citation type with no data behind it.

12 new tests across both repos (9 Relativity, 3 AIKB — see the security-test commits). Full suites green: 825/825 Relativity; 155/156 AIKB (the one pre-existing, unrelated failure noted since EL5, still not fixed there — out of scope).

### EL9 — Audit, budgets, and observability
- **Status: Substantially implemented (2026-07-31), across two passes.** See the Implementation Record below.
...
- **Deferred**: a user-facing lookup-history UI (listed as a Future feature, [§11](#11-additional-product-features)); `token_usage`/`used_in_final_answer` columns (see Implementation Record — a real, flagged deferral, not an oversight).

#### EL9 Implementation Record

**This milestone's core scope landed in two passes.** The `email_ingestion_events` extension and the audit write itself (per-call, naming the connection, origin, tool, result count, latency) were pulled forward narrowly during **EL7B**, since that milestone's own acceptance criteria required an audit row and full EL9 didn't exist yet to satisfy it — see EL7B's own Implementation Record for the schema/write-path detail (not repeated here). This pass (EL9 proper) added the remaining piece the Objective names but EL7B deliberately left out: the per-connection call-rate budget.

**Per-connection call-rate budget**: `Relativity/services/emailLiveLookupService.js#createInMemoryCallBudget` — an in-memory, sliding-window counter, one shared budget per connection across both tools (`search_email_messages`/`get_email_content` count against the same window, not two independent caps). Deliberately in-memory, matching this codebase's own existing rate-limiter posture exactly (`middleware/rateLimiters.js`'s `express-rate-limit` `MemoryStore`, backlog H6's own explicit "adequate for this app's current single-instance deployment" precedent) — a Redis-backed shared store was never in scope for the platform's other rate limiters either, so introducing one here alone would be inconsistent. Checked only *after* the 9-gate authorization chain succeeds — this budget tracks a legitimate, already-authorized user's own usage, not an anti-abuse gate (authorization already fully covers that). Exceeding it returns `{status:'error', reason:'rate_limited'}` — a reason §9's own table already named for a real Gmail 429, previously unreachable since `gmailService.js` can't detect that HTTP status; this is a second, independent path to the same named reason (Relativity's own budget, not Gmail's). A rejected call writes no audit row, matching the same "nothing was attempted against Gmail" convention every other gate rejection already follows. New config: `EMAIL_LIVE_LOOKUP_RATE_LIMIT_WINDOW_MS` (default 5 minutes), `EMAIL_LIVE_LOOKUP_RATE_LIMIT_MAX_CALLS` (default 20) — generous defaults, a cost/abuse backstop rather than a business limit a real user should ever hit in normal use.

**The security requirement's own "dedicated test" instruction** is satisfied directly: new tests prove a `live_lookup_fetch`/`search` audit row never contains the fetched message body, full subject, or search snippet text, even though the underlying tool call handled all three — asserting the audit row's exact allowed field set, not just spot-checking a few fields.

**`token_usage`/`used_in_final_answer` are deliberately NOT built**, despite being named in [§10.2](#102-fields-never-storing-full-message-bodies-by-default-per-the-tasks-explicit-requirement)'s field table. The architectural mismatch is real, not just extra effort: `emailLiveLookupService.js` writes its audit row at **tool-execution time** — before AIKB's *later* final-generation call (the one whose token usage would actually be attributed) has even happened. There is no clean way for AIKB to report that usage back into a row Relativity already wrote earlier without a new, separate round trip (e.g. an "update audit" call after generation completes) — exactly the "implementation detail to resolve at this milestone, not decided here" the Objective itself flagged as open. Given neither field is required by this milestone's own stated acceptance criteria ("every tool call... produces exactly one audit row; no audit row ever contains message content" — both already true), building a new cross-repo callback purely for two optional analytics columns was judged out of proportion to this pass — flagged here explicitly rather than silently shipped as "done."

10 new tests (this pass) plus the tests already landed with EL7B's own audit-write slice. Full suite green: 835/835 Relativity.
...
- **Not validated here, by design**: portal UI behavior (EL6) and Slack DM linking/lookup (EL7A/EL7B) — a short, non-blocking supplementary smoke check of both is recommended once EL6/EL7B ship, confirming the already-proven backend surfaces correctly through each UI, but this is not this milestone's job and should not delay it. **The Slack half of that smoke check now has a home: [EM10_5_SLACK_VALIDATION.md](EM10_5_SLACK_VALIDATION.md) Part D.** That document's primary purpose is broader and separate — validating Slack as a surface over already-*ingested* Gmail knowledge — and it explicitly does not discharge this milestone: EL10 remains the authoritative real-account and prompt-injection validation of the live-lookup **backend**, via direct signed calls. Running the Slack companion first neither satisfies nor blocks EL10.
