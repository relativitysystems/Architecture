...
**Status of this document: Proposed, with implementation underway. EL1–EL8 are implemented as of 2026-07-31** — architecture/contracts (EL1), the read-only tool registry (EL2, [§EL2](#el2--read-only-tool-registry)), the signed AIKB→Relativity execution endpoint (EL3, [§EL3](#el3--signed-aikb-to-relativity-execution-endpoint)), real Gmail search/content tools behind the 9-gate authorization chain (EL4, [§EL4](#el4--gmail-search-and-content-tools)), AIKB's bounded tool-calling orchestration (EL5, [§EL5](#el5--aikb-bounded-tool-orchestration)), the portal's mode selector/consent modal/live-source citations (EL6, [§EL6](#el6--portal-automaticlive-search-experience)), Slack identity linking (EL7A, [§EL7A](#el7a--slack-identity-linking)), Slack live-email access itself (EL7B, [§EL7B](#el7b--slack-live-email-access)), and citation-security hardening (EL8, [§EL8](#el8--citations-and-live-source-ui) — the "Live sources" UI itself was already built during EL6/EL7B; EL8 added the dedicated security tests §6 requires and an honest, flagged deferral of `live_email_attachment`). **EL9 onward remain unimplemented** — no soft token budgets or per-connection rate limits beyond EL7B's own narrow pulled-forward audit-write slice, and EL4–EL8 have not been validated against a real Gmail account (blocked on [EM10.5](EMAIL_INGESTION.md#em105--gmail-staging-validation), not yet run — see that milestone's own status). Treat each milestone's own Status line as authoritative over this banner if the two ever disagree.
