...
**Status of this document: Proposed, with implementation underway. EL1–EL7A are implemented as of 2026-07-31** — architecture/contracts (EL1), the read-only tool registry (EL2, [§EL2](#el2--read-only-tool-registry)), the signed AIKB→Relativity execution endpoint (EL3, [§EL3](#el3--signed-aikb-to-relativity-execution-endpoint)), real Gmail search/content tools behind the 8-gate authorization chain (EL4, [§EL4](#el4--gmail-search-and-content-tools)), AIKB's bounded tool-calling orchestration (EL5, [§EL5](#el5--aikb-bounded-tool-orchestration)), the portal's mode selector/consent modal/live-source citations (EL6, [§EL6](#el6--portal-automaticlive-search-experience)), and Slack identity linking (EL7A, [§EL7A](#el7a--slack-identity-linking) — the mapping only, no live-lookup behavior consumes it yet). **EL7B onward remain unimplemented** — Slack live-email access itself doesn't exist yet, and EL4–EL6 have not been validated against a real Gmail account (blocked on [EM10.5](EMAIL_INGESTION.md#em105--gmail-staging-validation), not yet run — see that milestone's own status). Treat each milestone's own Status line as authoritative over this banner if the two ever disagree.
