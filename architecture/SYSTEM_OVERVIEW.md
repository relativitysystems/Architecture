...
- **Scheduled jobs**: none exist, and none are planned, for Slack delivery. Relativity's Slack delivery-retry sweep (`GET /api/integrations/slack/sweep`) was originally designed as a Vercel Cron job, was unscheduled since production launch (the project's Vercel Hobby plan rejects a sub-daily cron schedule), and has since been **removed outright** along with its route and auth code. In its place, bounded, immediate, in-flow delivery retries followed by a terminal `delivery_failed` status are implemented and require no scheduler at all — see [ADR-007](../decisions/ADR-007-SLACK-BOUNDED-DELIVERY-RETRY.md), [CONNECTOR_FRAMEWORK.md](CONNECTOR_FRAMEWORK.md), and [../roadmap/FEATURE_BACKLOG.md](../roadmap/FEATURE_BACKLOG.md).
...
- **Slack delivery has no scheduled recovery of any kind, by design, and does not guarantee eventual delivery.** Bounded, immediate, in-flow delivery retries with a terminal `delivery_failed` state are implemented in place of the removed sweep — see [ADR-007](../decisions/ADR-007-SLACK-BOUNDED-DELIVERY-RETRY.md). A sustained outage longer than the retry window can still lose an answer; the user must ask again. AIKB-side redaction of chat content on `delivery_failed` is best-effort and can silently fail with no automatic retry. See [CONNECTOR_FRAMEWORK.md](CONNECTOR_FRAMEWORK.md).
