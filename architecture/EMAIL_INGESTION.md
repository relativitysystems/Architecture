...
**Status of this document: Proposed, Partially Implemented (EM1–EM5).** As of 2026-07-24, **EM1 — Multi-member schema foundation (§31)**, **EM2 — Gmail OAuth per member (§31)**, **EM3 — Organization policy engine (§31)**, **EM4 — Member mailbox settings (§31)**, and **EM5 — Gmail label workflow (§31)** are all implemented in the Relativity repository — see the Implementation Records immediately below. Every other milestone, **EM6 through EM13, remains unimplemented**: no real ingestion into AIKB, no sync/scheduling, and no Microsoft/Outlook adapter. EM5 creates/reuses the managed Gmail label and runs a fully policy-evaluated, never-persisted dry-run preview — it never fetches a message body, ingests anything, or writes to AIKB. Every claim about *current* behavior is sourced from a specific file (cited inline); everything else remains a proposal. This document does not implement the feature beyond EM1's schema, EM2's OAuth connect flow, EM3's policy engine, EM4's member settings surface, and EM5's label workflow/preview — see [Constraints](#constraints-carried-into-this-plan) below.
