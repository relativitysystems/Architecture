...
**Status of this document: Proposed, Partially Implemented (EM1–EM2).** As of 2026-07-23, **EM1 — Multi-member schema foundation (§31)** and **EM2 — Gmail OAuth per member (§31)** are both implemented in the Relativity repository — see the Implementation Records immediately below. Every other milestone, **EM3 through EM13, remains unimplemented**: no organization policy engine, no Gmail label workflow, no ingestion, no sync/scheduling, and no Microsoft/Outlook adapter. EM2 itself is connect/status/disconnect only, exactly as scoped — no policy evaluation, no managed label, no message reading of any kind. Every claim about *current* behavior is sourced from a specific file (cited inline); everything else remains a proposal. This document does not implement the feature beyond EM1's schema and EM2's OAuth connect flow — see [Constraints](#constraints-carried-into-this-plan) below.
