...
**Status of this document: Proposed, Partially Implemented (EM1–EM6).** As of 2026-07-24, **EM1 — Multi-member schema foundation (§31)**, **EM2 — Gmail OAuth per member (§31)**, **EM3 — Organization policy engine (§31)**, **EM4 — Member mailbox settings (§31)**, **EM5 — Gmail label workflow (§31)**, and **EM6 — Historical import using labeled messages (§31)** are all implemented, in both the Relativity and AIKB repositories for EM6 — see the Implementation Records immediately below. Every other milestone, **EM7 through EM13, remains unimplemented**: no incremental cursor sync, no automatic-mode/scheduler, no attachments, and no Microsoft/Outlook adapter. EM6 is the first milestone that actually writes to AIKB and to a client's searchable knowledge base — real, `manual_selected`-mode-only historical import, paginated, policy-evaluated, and normalized before upload. Every claim about *current* behavior is sourced from a specific file (cited inline); everything else remains a proposal. This document does not implement the feature beyond EM1's schema, EM2's OAuth connect flow, EM3's policy engine, EM4's member settings surface, EM5's label workflow/preview, and EM6's historical import — see [Constraints](#constraints-carried-into-this-plan) below.
