...
**Status of this document: Proposed, Partially Implemented (EM1–EM7).** As of 2026-07-24, **EM1 — Multi-member schema foundation (§31)**, **EM2 — Gmail OAuth per member (§31)**, **EM3 — Organization policy engine (§31)**, **EM4 — Member mailbox settings (§31)**, **EM5 — Gmail label workflow (§31)**, **EM6 — Historical import using labeled messages (§31)**, and **EM7 — Incremental Gmail sync (§31)** are all implemented, in both the Relativity and AIKB repositories for EM6 (EM7 is Relativity-only) — see the Implementation Records immediately below. Every other milestone, **EM8 through EM13, remains unimplemented**: no automatic-mode/scheduler, no attachments, and no Microsoft/Outlook adapter. EM6 was the first milestone that actually writes to AIKB and to a client's searchable knowledge base; EM7 makes repeat syncs cheap by reading only what changed since the last sync (Gmail's History API) instead of re-scanning the whole label every time, falling back to a full re-scan when the stored cursor goes stale. Every claim about *current* behavior is sourced from a specific file (cited inline); everything else remains a proposal. This document does not implement the feature beyond EM1's schema, EM2's OAuth connect flow, EM3's policy engine, EM4's member settings surface, EM5's label workflow/preview, EM6's historical import, and EM7's incremental sync — see [Constraints](#constraints-carried-into-this-plan) below.
...
## EM5 Implementation Record

Implemented 2026-07-24, in `relativitysystems/Relativity` only (EM5 has no AIKB-side work — §31's EM5 entry lists only Relativity, `DB: none new` — uses EM1's `email_connections.managed_label_id`). Scope exactly as described there: managed-label create-or-reuse on connect, the label-query dry-run preview (`POST /connections/:id/preview`), and the pure label-removal reconciliation logic (§24.2) exercised via fixtures — **still no message fetch beyond metadata headers, no ingestion, and nothing is ever persisted to AIKB.** No live browser OAuth/Gmail-API walkthrough was performed against a real Google Cloud OAuth Client — every test below is dependency-injected against a fixture "mailbox," matching this repo's existing no-live-network-call testing convention (the same limitation the EM2 record already documents for the OAuth flow itself, now extended to every live Gmail API call this milestone adds).

**Source references:**

- `Relativity/services/gmailService.js` (extended) — `REQUIRED_SCOPES` gains `gmail.labels` (§12 item 6); new `getOrCreateManagedLabel`/`listLabels` (idempotent managed-label lookup/creation, §10); new `refreshAccessToken` (raw token-refresh call only — orchestration lives in `emailConnectionService.js`); new `listMessageIdsByQuery`/`getMessageMetadata` (bounded, metadata-only — headers `Subject`/`From`/`Date` plus `labelIds`, **never the message body**, §14.1's "never body content" requirement); new pure `compileSearchQuery` (Gmail search-string compiler, §10/§17) and `extractEmailAddress`/`parseMessageHeaders` header-parsing helpers.
- `Relativity/services/emailConnectionService.js` (extended) — `handleCallback` (EM2) now calls `getOrCreateManagedLabel` with the fresh access token immediately after a successful token exchange, best-effort (a label failure does not fail the connection — see deviation 1); new `emailConnectionsRepo.updateManagedLabelId`; new exported `getValidGmailAccessToken` (refresh-if-expiring-or-expired orchestration, persisting via the existing, unchanged `oauthConnectionsService.updateCredentialForConnection`, preserving the prior refresh token when Google's refresh response omits a new one — the same bug class ADR-006 already solved for Google Drive); new exported `ensureManagedLabel` (lazy backfill for a connection whose connect-time label creation failed) and `getEmailConnectionRecord` (read accessor for the new preview service).
- `Relativity/services/emailPreviewService.js` (new) — `createEmailPreviewService(deps)` factory exporting `buildPreview` (the dry-run orchestration: compile query → list a bounded candidate page → fetch metadata → re-verify each candidate locally via EM3's `evaluateMessageAgainstPolicy`, never a rule-composition of its own) and the pure, DB-free `computeLabelReconciliation` (§24.2 — built now, fixture-tested, so EM6 can call it unmodified once real `email_ingestion_events` rows exist).
- `Relativity/routes/integrations/email.js` (extended) — `POST /connections/:id/preview`: self-service only, reusing `canDisconnectConnection` for the ownership check (identical shape to disconnect/sync-mode); resolves a valid access token via `getValidGmailAccessToken` (mapping `AUTHORIZATION_EXPIRED` to a `400` "please reconnect" response, never a raw Gmail error); calls `ensureManagedLabel` for any non-`automatic` connection before building the preview, since only those need `managed_label_id` resolved for the label gate to be trustworthy.
- `Relativity/public/portal/portal.html`, `portal.js`, `portal.css` (extended) — inside the existing Gmail card, a new shell shown only while connected: manual-mode-only label instructions plus an "Open Gmail" deep link (`https://mail.google.com/mail/u/0/#label/Relativity%2FKnowledge`, hidden in `automatic` mode since the label plays no role there, §16.1 item 3), and a "Sync now (preview)" button calling the new preview route and rendering `{matched}/{scanned}` plus a subject/from sample list, with a "Load more" control that walks `nextPageToken` and accumulates totals across pages rather than replacing them.
- `Relativity/test/gmailService.test.js` (extended, +33 tests) — updated scope-assertion tests for `gmail.labels`' addition (§12 item 6); full coverage of `refreshAccessToken` (including the "Google omits the new refresh token" case and secret-non-leakage), `listLabels`/`getOrCreateManagedLabel` (reuse-vs-create, create-failure), `listMessageIdsByQuery`/`getMessageMetadata` (pagination passthrough, fail-closed `null`-query short-circuit, metadata-only — never `format=full`), `extractEmailAddress`/`parseMessageHeaders`, and `compileSearchQuery` (manual mode's fixed label query regardless of policy; automatic mode's allow-rule-OR compilation; deny rules never compiled in; disabled rules skipped; the fail-closed `null` return for zero/non-compilable allow rules).
- `Relativity/test/emailConnectionService.test.js` (extended, +19 tests) — `handleCallback`'s managed-label creation (persisted on success, non-fatal on failure); the full `getValidGmailAccessToken` matrix (fresh token reused without a refresh call; expiring/missing-`expiresAt`/already-expired tokens refreshed; the prior refresh token preserved vs. the rotated one used; `AUTHORIZATION_EXPIRED` for a missing credential, a missing fallback refresh token, and a failed refresh attempt); `ensureManagedLabel` (no-op when already set, create-and-persist when not) and `getEmailConnectionRecord`.
- `Relativity/test/emailPreviewService.test.js` (new, 18 tests) — the full §28.1 label-workflow suite this milestone owns: one/multiple labeled emails matched; bulk pagination across two calls with no double-processing; unlabeled-but-policy-matching messages excluded in manual mode (proving the local defense-in-depth re-check, §17 item 4, independent of whatever the real provider query would have returned); automatic mode ignoring label presence/absence entirely; deny-overrides-allow even with a label present; the baseline allow-and-label positive path; a labeled-but-out-of-policy message excluded; empty policy failing closed in **both** modes (automatic short-circuits with zero provider calls; manual still lists the label query but excludes everything locally); the exact `{matchedCount, scannedCount, sample, nextPageToken, complete}` response shape with `sample` entries as exactly `{subject, from, date}`; sample capping; two concurrently-run previews never sharing state; and `computeLabelReconciliation`'s fixture-driven tombstone/keep/mixed/empty-input cases.
- `Relativity/test/emailRoutes.test.js` (extended, +1 test) — `POST /connections/:id/preview` rejects an unauthenticated request with `401`, matching every other route in this file.
- Full existing suite plus all EM5 additions: `Relativity` 521/521 (`npm test`).

**Deviations / implementation decisions where this document was silent, with reasons:**

1. **Managed-label creation at connect time is best-effort — a Gmail `labels.create`/`labels.list` failure does not fail the connection.** §10 describes the label as created "on first successful connect" but doesn't say what happens if that specific call fails independently of the OAuth exchange it rides on. Failing the whole connection over a label-creation hiccup would be a worse outcome than a member ending up connected-but-labelless, since `ensureManagedLabel`'s lazy backfill (next deviation) recovers automatically on the member's first preview/sync attempt — treated the same "don't let a secondary failure sink the primary operation" pattern this codebase already uses for `disconnect`'s best-effort provider-side token revocation.
2. **A lazy `ensureManagedLabel` backfill path was added, not specified by this document.** Necessary consequence of deviation 1: something has to eventually create the label for a connection where connect-time creation failed, and `POST /connections/:id/preview` (the first thing that ever needs `managed_label_id` after connect) is the natural, cheapest place — idempotent and free (no network call) once the label exists, so it costs nothing on the common path.
3. **§14.1's route table describes manual mode's preview query as "`label:Relativity/Knowledge` plus policy" — this implementation compiles it as exactly `label:Relativity/Knowledge -in:chats`, with policy enforced only by the local re-check, not folded into the provider query string.** §17 item 2 (Historical Import Design) is unambiguous that "the query is simply `label:Relativity/Knowledge`... the label, not a date window, is the primary selection criterion," directly contradicting §14.1's own looser one-line paraphrase. Resolved the same way EM3's deviation 6 resolved an analogous §14.1-vs.-detailed-section inconsistency: in favor of the more specific, mechanism-focused section. Correctness is unaffected either way — `evaluateMessageAgainstPolicy` still fully enforces policy per candidate message; only *where* that enforcement happens (provider query vs. local check) differs, and the local-check answer is simpler, more testable, and matches §10 item 4's own defense-in-depth framing ("deny-list criteria are still re-verified locally in both modes").
4. **Automatic mode's compiled query only ever uses a rule's `labelOrFolder`/`senderPattern` — never `subjectKeyword`/`recipientPattern`, and deny rules are never compiled in at all.** §16.1's own MVP field-subset table already excludes `subjectKeyword`/`recipientPattern` from the rule-builder UI, so no rule the UI can produce would ever need them compiled; a rule created directly via the API with one of those fields simply doesn't get that criterion pre-filtered at the provider layer (the local check still evaluates it, per `ruleMatchesMessage`'s unchanged EM3 logic — it just can't be *positively selected for* by the query, which fails closed, not open). Deny rules are excluded from query compilation because §10 item 4 already establishes deny is local-only ("still re-verified locally"), and safely negating an arbitrary deny clause inside a larger OR'd allow-query is a materially harder query-composition problem this milestone doesn't need to solve given the local check already covers it.
5. **Preview never fetches `To`/`Cc` headers**, even though `ruleMatchesMessage` (EM3) supports `recipientPattern` matching. Same reasoning as deviation 4: `recipientPattern` is UI-deferred (§16.1), so no rule the portal can create ever sets it; omitting the extra header fetch costs nothing today and keeps each candidate message to exactly the three headers §14.1's "never body content" requirement already implies are the ceiling. A rule with `recipientPattern` set (API-only) simply never matches on that criterion — fail-closed, not a silent false-positive.
6. **`getValidGmailAccessToken` (token-refresh orchestration) is new — this document's §12 items 3-4 describe *that* refresh must happen and *which* existing primitive (`updateCredentialForConnection`) to persist it with, but not *what decides when to call it*.** EM2 never needed this (the connect flow's token is always fresh, seconds old); EM5 is the first milestone making a live Gmail call an arbitrary amount of time after connect, so it's the first that actually needs a refresh-if-expiring check. A 5-minute-before-expiry margin was chosen defensively (clock skew plus the preview call's own runtime) — not specified anywhere in this document, a reasonable engineering default.
7. **The sync-mode → preview-mode mapping treats `paused` identically to `manual_selected`** (label-gated query), since `email_connections.sync_mode`'s three-value enum (§13.1) has no fourth "what would this connection do" state and `paused` isn't reachable via the API yet (EM4's own flagged pause/resume gap — no route exists to ever set it). Revisit once EM4's gap is filled; this is a default for an unreachable state today, not a considered design choice for a real one.

**Testing summary:** see the Source references above for the full breakdown by file; 33 new tests in `gmailService.test.js`, 19 new tests in `emailConnectionService.test.js`, 18 new tests in `emailPreviewService.test.js`, 1 new test in `emailRoutes.test.js`, 521/521 passing across the full suite.

**Not implemented (deliberately, per EM5's own scope):** any real ingestion, message-body fetch, or write to AIKB (EM6); the Gmail History API incremental cursor (EM7 — this milestone always reads current label/candidate state directly, per §18.4); the `pause`/`resume` route/control (§14.1, §7 — still the same EM4-flagged gap, unclaimed by any milestone); Microsoft/Outlook (EM12). **No live browser OAuth flow or real Gmail API call was exercised** — every claim here (label idempotency, query compilation, metadata parsing, token refresh) is verified through dependency-injected unit tests against a fixture mailbox, not a live Google Cloud OAuth Client walkthrough, consistent with this repo's existing no-test-database, no-real-network-call convention and the same limitation the EM2 record already documents for the OAuth exchange itself.

---

## EM6 Implementation Record

Implemented 2026-07-24, in both `relativitysystems/Relativity` and `relativitysystems/aikb`. Scope exactly as described in §31's EM6 entry: first real ingestion — bounded, paginated, `manual_selected`-mode-only historical import, using EM5's label workflow, forwarding normalized text to AIKB's existing, generalized `/ingest` route. **EM6 is deliberately narrower than §17's own general text**, which describes historical import as running "in either sync mode": this implementation compiles only the fixed `manual_selected` label query and rejects a sync attempt on an `automatic`-mode connection with a dedicated `AUTOMATIC_SYNC_NOT_SUPPORTED` error, matching §31's own EM6 Backend line ("historical path, manual mode only") rather than §17's looser paraphrase — the same class of resolution EM3's deviation 6 and EM5's deviation 3 each already made for an analogous §14.1-vs-detailed-section inconsistency. Automatic-mode historical/incremental sync remains EM7/EM8's responsibility, once the tick scheduler and its own safeguards exist.

**Source references:**

- `aikb/routes/knowledge.js` (extended) — `POST /ingest`'s `sourceProvider` allow-list widened from `{'portal_upload'}` to `{'portal_upload', 'gmail', 'microsoft'}` (§14.2); new optional `collectionId`/`emailMetadata` payload fields, threaded into the `knowledge/document.ingest` Inngest event; `emailMetadata` is required (400 otherwise) whenever `sourceProvider` is `gmail`/`microsoft`.
- `aikb/inngest/functions.js` (extended) — `ingestDocument`'s `sourceProvider` check widened identically; the **cross-file duplicate-content-hash skip is disabled for `sourceProvider IN ('gmail','microsoft')`** (§20) via a small explicit `isEmailSource` branch immediately around the existing `getIndexedDocumentByContentHash` call — the same-file, unchanged-content skip (`existing.content_hash`) is untouched and remains correct for email; an explicit `collectionId` (a matched `email_ingestion_rules.destination_collection_id`, §22) now takes priority over the default-collection lookup at first insert; a new, additive `write-email-source-message` step runs immediately after the existing `mark-indexed` step (never replacing or reordering any existing step) whenever `sourceProvider` is email and `emailMetadata` is present, populating `email_source_messages` via the new `upsertEmailSourceMessage`. The unchanged-hash skip path (re-syncing an already-ingested, content-identical message) intentionally does **not** reach this new step — see deviation 1.
- `aikb/services/supabaseService.js` (extended) — new `upsertEmailSourceMessage(clientId, documentId, emailMetadata, contentHash)`, upserted on `document_id` (not inserted — a re-sync with changed label/folder metadata but an unchanged body still needs to refresh it, though EM6's own caller only reaches this on a true first-insert/content-changed ingest, per the note above).
- `aikb/test/knowledgeIngestRoute.test.js` (new) — HTTP-level auth-gating and `sourceProvider`/`emailMetadata` validation tests, mirroring `test/knowledgeAskRoute.test.js`'s established convention (only paths that resolve before any real Supabase/Inngest call are exercised).
- `relativity/services/emailNormalizationService.js` (new) — the full §19 pipeline as pure, dependency-free functions: `htmlToPlainText`, `truncateAtHtmlQuoteBoundary` (structural HTML-level quote cutting at `<blockquote>`/`gmail_quote`), `stripQuotedReplies` (text-level defense-in-depth: `On ... wrote:`, `-----Original Message-----`, leading `>` lines), `stripSignature` (a `--`/`__` delimiter or a short trailing disclaimer-phrased block), and `normalizeEmailBody` orchestrating all of them. Every stripping step falls back to its pre-strip input rather than ever returning empty.
- `relativity/services/gmailService.js` (extended) — new `getMessageBody({accessToken, messageId})` (`format=full`, the only call in this file that ever fetches body content — §19: "raw HTML/raw MIME never stored," enforced by every caller passing this straight through `emailNormalizationService` before it is ever persisted or forwarded), plus the pure `decodeBase64Url`/`extractBodyParts` helpers (recursive MIME-part walk, skipping any `filename`-carrying part regardless of its declared MIME type — an attachment, never a body, per §21's "deferred past MVP" scoping). `getMessageMetadata` gains a `threadId` field (Gmail's response always carries it at the top level) to populate `email_source_messages.provider_thread_id` for future UI thread-grouping (§20) — EM5's preview never needed it.
- `relativity/services/aikbService.js` (extended) — `uploadAndIngest` gains optional `sourceProvider`/`collectionId`/`emailMetadata` parameters, fully backward-compatible (every existing `portal_upload` caller in `routes/api.js` passes none of them); new `deleteDocumentById(clientId, documentId)` — deliberately id-based, not by-source, and deliberately omits `sourceProvider` from its payload, since AIKB's `DELETE /document/:id` route only consults that field for its own by-source validation branch, which an id-based call never takes. This is why **no AIKB change was needed for deletion/tombstoning at all**, matching §14.2's own claim verbatim — `services/emailSyncService.js`'s reconciliation resolves the real AIKB document id via `listDocuments` first (since `/ingest` never returns one synchronously — it only enqueues an async Inngest event) rather than requiring AIKB's delete route to accept a `sourceProvider` it was never designed to validate against an id-based lookup.
- `relativity/services/emailSyncService.js` (new) — `createEmailSyncService(deps)` factory exporting `syncConnection`, the full §15/§17 historical-import orchestration: the connection-level fail-closed gate (`assertSyncAllowed` — `search_enabled`, `sync_enabled`, `paused`, and the EM6-scope `automatic` rejection described above, each its own named error code); the existing per-client `MAX_DOCUMENTS` ceiling check (§17 item 6), run once per fresh sync (`pageToken` absent), not re-checked on a resumed page; one bounded page (`HISTORICAL_PAGE_SIZE = 25`) of `listMessageIdsByQuery` → per-candidate `getMessageMetadata` + local policy re-verification (reusing EM3's `evaluateMessageAgainstPolicy` and EM5's label-gate pattern unmodified) → (if eligible) `getMessageBody` → `emailNormalizationService.normalizeEmailBody` → `aikbService.uploadAndIngest`; per-message failure isolation (a body-fetch, normalization, or AIKB-ingest failure is caught, recorded `outcome=failed`, and the page continues — §17 item 5); `email_sync_runs`/`email_ingestion_events`/`email_connections.historical_import_status`/`email_sync_state` bookkeeping via an injectable `emailSyncRepo`; and, once a sync reports `complete: true`, the §24.2 label-removal reconciliation pass (a single bounded `RECONCILIATION_LIST_SIZE = 500` re-listing of every message currently under the label, diffed against every message this connection has ever recorded `outcome='ingested'`, tombstoning anything no longer labeled via `aikbService.deleteDocumentById`) — best-effort and non-fatal to the sync run that triggered it.
- `relativity/routes/integrations/email.js` (extended) — `POST /connections/:id/sync`: self-service only, identical ownership shape to preview/sync-mode (`canDisconnectConnection`); resolves a valid access token and, for non-`automatic` connections, the managed label (reusing EM5's `getValidGmailAccessToken`/`ensureManagedLabel` exactly); maps `emailSyncService.ERROR_CODES` to a `400` with `{error, code}`, everything else to a `500`. Body: optional `{pageToken}`. Response: the full `{syncRunId, status, complete, nextPageToken, imported, skipped, failed, reconciled, errorSummary}` summary.
- `relativity/public/portal/portal.html`/`portal.js` (extended) — the EM5 "Sync now (preview)" button and copy are replaced with real "Sync now": each click drives `POST /connections/:id/sync` to completion automatically (accumulating `{imported, skipped, failed}` totals across pages, following `nextPageToken` without a manual "Load more" click — unlike EM5's preview, a member starting a real sync wants it to finish), bounded by a client-side `EMAIL_SYNC_MAX_AUTO_PAGES = 40` safety cap so a pathological loop can never hang the tab indefinitely.
- `relativity/test/emailNormalizationService.test.js` (new, 24 tests) — the full §28 "Unit — normalization" table: HTML→text, entity decoding, script/style stripping, image alt-text preservation; structural (`<blockquote>`/`gmail_quote`) and text-level (`On ... wrote:`, `-----Original Message-----`, leading `>`) quote stripping, including a real-shaped multi-reply Gmail HTML fixture and an Outlook-style HTML fixture; signature stripping including the "entirely a signature block" and "no quoted content" (no-op) edge cases explicitly called for; forwarded-message handling.
- `relativity/test/gmailService.test.js` (extended, +14 tests) — `getMessageBody`/`extractBodyParts`/`decodeBase64Url`: simple single-part, `multipart/alternative`, nested `multipart/mixed` > `multipart/alternative`, a filenamed part never treated as a body regardless of declared MIME type, empty/missing payloads, `format=full` request shape, HTTP-error mapping, required-argument checks.
- `relativity/test/emailSyncService.test.js` (new, 26 tests) — `assertSyncAllowed`'s four fail-closed gates plus the healthy-connection non-throw case; the same gates surfaced through a real `syncConnection` call (asserting zero Gmail calls happen first); the document-limit gate (first page only, not on resume); the full Policy Evaluation Model re-exercised at this orchestration layer (empty policy, unlabeled-but-matching, labeled-but-out-of-policy, deny-overrides-allow); the successful ingest path's exact `emailMetadata` shape and `collectionId` routing; two distinct near-identical messages both ingesting independently (dedup-narrowing awareness); per-message failure isolation (body-fetch failure, empty-after-normalization, AIKB-ingest failure) each continuing the rest of the page; pagination/resume across two calls; cross-member isolation (two connections' syncs never cross, each `contributingMemberId` correct); label-removal reconciliation (tombstones a no-longer-labeled message, leaves a still-labeled one untouched, never runs on an incomplete page); a page-level failure marking the run `failed` with an `errorSummary` rather than throwing.
- `relativity/test/emailRoutes.test.js` (extended, +1 test) — `POST /connections/:id/sync` rejects an unauthenticated request with `401`, matching every other route in this file.
- Full existing suites remain green: `relativity` 582/582 (`npm test`); `aikb` unaffected by this change (2 pre-existing, unrelated environment-isolation failures in `runKnowledgeQuery.test.js`/`slackEventsRetired.test.js` — confirmed via `git stash` to fail identically on `main` before this milestone's changes, not introduced by it).

**Deviations / implementation decisions where this document was silent, with reasons:**

1. **The new `write-email-source-message` Inngest step only runs on a true first-insert/content-changed ingest, never on the existing unchanged-hash skip path.** §14.2 says the step runs "as an additive step after the existing mark-indexed step" without addressing the skip path (which returns before `mark-indexed` is ever reached). Re-syncing an unchanged message leaves its existing `email_source_messages` row as-is rather than re-writing identical data — a deliberate, minor scope narrowing consistent with the step being additive, not a new always-run write path.
2. **`DELETE /api/knowledge/document/:id` required no AIKB-side change**, resolving what first read as a discrepancy between §14.2's explicit "no AIKB change needed here at all" claim and the route's existing `sourceProvider !== 'portal_upload'` validation gate. Tracing the actual code: that gate runs unconditionally regardless of whether the caller deletes by real document UUID or by-source, but the `documentId` lookup branch never consults the payload's `sourceProvider` value for anything beyond that gate — so a caller that deletes by real id and simply omits `sourceProvider` (defaulting server-side to `portal_upload`) passes the check trivially regardless of the document's actual provider. `services/emailSyncService.js`'s reconciliation therefore always resolves the real AIKB document id first (via `aikbService.listDocuments`, matching on `source_provider`/`source_file_id`) and deletes by id, exactly as §14.2 assumes, rather than ever calling the by-source delete path for email.
3. **EM6 rejects `automatic`-mode sync attempts outright (`AUTOMATIC_SYNC_NOT_SUPPORTED`), not merely "not yet automatic" — see the record's opening paragraph.** A real, deliberate scope cut favoring §31's EM6 Backend line over §17 item 2's more general "in either sync mode" phrasing, for the same reasons EM3/EM5 already established a precedent for resolving this exact kind of internal inconsistency.
4. **The label-removal reconciliation pass (§24.2) uses a single bounded `RECONCILIATION_LIST_SIZE = 500` listing of "currently labeled messages," not true unbounded pagination.** §24.2 doesn't specify a bound for this specific listing (unlike §17 item 3's explicit historical-import pagination requirement). A mailbox with more than 500 labeled messages gets a best-effort reconciliation pass on any one sync — a documented MVP bound, not a correctness bug, since the next completed sync re-attempts the same diff against then-current state.
5. **`email_ingestion_events.ingested_document_id` is populated by reconciliation (via the id resolved from `aikbService.listDocuments`) but is never populated by the ingest path itself**, since `/ingest` only enqueues an async Inngest event and never returns a real document id synchronously to Relativity (§15's own sequence diagram records `outcome=ingested` immediately after the enqueue call, before any actual embedding has happened). This is consistent with the sequence diagram's intent — "ingested" here means "successfully queued," not "confirmed embedded" — but means a query against `email_ingestion_events` for an `ingested` row's `ingested_document_id` will find it null until/unless that same message is later reconciled. Flagged as a minor, acceptable observability gap rather than solved with a synchronous AIKB round trip this milestone doesn't otherwise need.
6. **A message that normalizes to empty text (e.g. an image-only or entirely-boilerplate email) is recorded `outcome=failed`, not silently skipped or ingested as an empty document**, matching the existing document-ingestion precedent (`aikb/inngest/functions.js`'s `if (!parsedText || !parsedText.trim()) throw new Error('Parsed document produced no text')`) applied one layer earlier, before the AIKB round trip, so the empty-body case is visible in `email_ingestion_events` rather than only surfacing as a later, less-specific AIKB job failure.
7. **The portal's "Sync now" button drives pagination to completion automatically, rather than requiring a manual "Load more" click the way EM5's preview did.** Neither §7 nor §7.1 specifies this UX choice explicitly; a preview is inherently exploratory (a member deciding whether to sync at all benefits from controlling how much they scan), while a real sync a member has already chosen to run should just finish — bounded by a client-side safety cap (`EMAIL_SYNC_MAX_AUTO_PAGES = 40`, ~1000 messages) so a pathological server response can never hang the tab indefinitely.

**Testing summary:** see the Source references above for the full breakdown by file; 24 new tests in `emailNormalizationService.test.js`, 14 new tests in `gmailService.test.js`, 26 new tests in `emailSyncService.test.js`, 1 new test in `emailRoutes.test.js` (Relativity); a new `knowledgeIngestRoute.test.js` (AIKB). `relativity` 582/582 passing across the full suite.

**Not implemented (deliberately, per EM6's own scope):** the Gmail History API incremental cursor — every sync is a full bounded historical re-scan against the label query, `email_sync_state.provider_cursor`/`cursor_status` are written but not yet consulted (EM7); automatic-mode sync of any kind, including the tick scheduler (EM8); member offboarding reconciliation (EM9); production-quality normalization/threading/citations — `provider_thread_id` is captured but not yet surfaced in citations, and `generateRagAnswer`/`runKnowledgeQuery`'s citation formatting is unchanged (EM10); attachments — `hasAttachments` is always recorded `false`, `include_attachments` policy toggle remains inert (EM11); Microsoft/Outlook (EM12); the `pause`/`resume` route/control (still the same EM4-flagged gap, unclaimed by any milestone). **No live browser OAuth flow or real Gmail/AIKB round trip was exercised** — every claim in this record (normalization correctness, MIME parsing, policy re-evaluation, pagination, reconciliation, cross-member isolation) is verified through dependency-injected unit tests against fixture mailboxes and a fixture AIKB client, not a live Google Cloud OAuth Client or live AIKB Supabase/Inngest walkthrough, consistent with this repo's existing no-test-database, no-real-network-call convention and the same limitation every prior EM record in this document already documents.

---

## EM7 Implementation Record

Implemented 2026-07-24, in `relativitysystems/Relativity` only (EM7 has no AIKB-side work — history.list/getProfile both fall under the existing `gmail.readonly` scope, no AIKB change needed). Scope exactly as described in §31's EM7 entry: `email_sync_state` cursor usage so subsequent manual syncs are incremental, not full re-scans, still user-triggered — falling back transparently to a bounded historical re-scan when the cursor has expired (§18.4), and now correctly reporting label-changed diffs for reconciliation instead of EM6's direct-read/full-list approach.

**Source references:**

- `relativity/services/gmailService.js` (extended) — `getMailboxHistoryId` (`users.getProfile`, establishes a fresh cursor once a sync completes) and `listHistory` (`users.history.list`, scoped via the `labelId` query param to the managed label — EM7 stays `manual_selected`-only, §18.2), both under the existing `gmail.readonly` scope; a new `ERROR_CODES.HISTORY_EXPIRED`, thrown specifically on Gmail's documented `404` for a `startHistoryId` older than the retained history window, distinct from a generic `HTTP_ERROR` so callers can tell "fall back to historical" from "a real failure" apart. `listHistory` returns a flat, order-preserving `{type, messageId}` change list (`labelAdded`/`labelRemoved`/`messageDeleted`) rather than pre-grouped sets — callers reduce it to one final action per message themselves (see the next bullet), since a message can legitimately be labeled and unlabeled again within the same page. `getMessageMetadata` gains a `threadId` field (Gmail always returns it), populated but not yet consumed by anything before EM10.
- `relativity/services/emailSyncService.js` (substantially extended) — `syncConnection` now decides `historical` vs `incremental` at the start of every FRESH call (never a resumed/paginated one, which trusts the caller's `runType` instead): it reads `email_sync_state` via the new `emailSyncRepo.getSyncState`, and if a valid cursor exists AND the connection has a managed label, PROBES `gmailService.listHistory` immediately (before ever creating a sync-run row) — a successful probe becomes the incremental path's first page (reused, not re-fetched); a `HISTORY_EXPIRED` result calls the new `emailSyncRepo.markCursorExpired` and falls back to historical, all within the same click. The historical/incremental page processors were refactored to share one `processCandidateMessage`/`processOneCandidate` eligibility-and-ingest pipeline (metadata → policy re-verification → body fetch → normalize → AIKB forward) and one `tombstoneMessages` helper, so the two paths can never drift apart. Historical still runs `reconcileRemovedLabelsFullList` (EM6's bounded full-list diff) once complete; incremental never does — `labelRemoved`/`messageDeleted` entries from the diff are tombstoned directly, per page, as they're seen (§18.4's own framing: the diff **is** the reconciliation mechanism). On completion of either type, a fresh cursor is captured — incremental reuses the last page's own `historyId`; historical calls `getMailboxHistoryId` explicitly — and `email_sync_state` is upserted via a new conditional-write contract (`providerCursor`/`cursorStatus`/`cursorObtainedAt` default to `undefined`, meaning "leave unchanged," so an incomplete or failed run's bookkeeping call never clobbers a still-valid cursor with nothing). New `listSyncRuns`, backing the sync-run history view.
- `relativity/routes/integrations/email.js` (extended) — `POST /connections/:id/sync` now also accepts/returns `runType` (required together with `pageToken` when resuming a paginated run — the route rejects an inconsistent resume the same way the service does, via `ERROR_CODES.INVALID_RESUME`); new `GET /connections/:id/sync-runs`, self-service, same ownership shape as every other connection-scoped route in this file.
- `relativity/public/portal/portal.html`/`portal.js`/`portal.css` (extended) — "Sync now"'s pagination loop now threads `runType` through every resumed page (not just `pageToken`); the status line distinguishes "Quick check" (incremental) from "Full scan" (historical), matching §31's EM7 Frontend line ("'Sync now' button reflects incremental vs. historical run type"); a new sync-run history list (`GET .../sync-runs`, loaded on connect and refreshed after every sync) — type, status, imported/failed counts, and timestamp per run.
- `relativity/test/gmailService.test.js` (extended, +14 tests) — `getMailboxHistoryId`/`listHistory`: request shape (`startHistoryId`/`labelId`/`pageToken`/all three `historyTypes`), flattening `labelsAdded`/`labelsRemoved`/`messagesDeleted` into an ordered change list, record-order preservation for a message appearing more than once, `HISTORY_EXPIRED` specifically on `404` vs. generic `HTTP_ERROR` on other non-2xx statuses, empty-page handling, required-argument checks.
- `relativity/test/emailSyncService.test.js` (extended, +16 tests, all EM6 tests retained and still passing against the refactored implementation) — cursor establishment after a complete historical sync (and that it is NOT established while incomplete, or after a failed run); incremental sync chosen over historical when a valid cursor exists, scoped via `labelId`, never falling back to a full `listMessageIdsByQuery` scan; `labelAdded` running through the identical eligibility/ingest pipeline as historical (both success and policy-skip cases); `labelRemoved`/`messageDeleted` tombstoning directly from the diff with distinct reason text; the same-page labeled-then-unlabeled last-write-wins case; cursor-expiry fallback (both the successful fallback-and-recover path and that `markCursorExpired` is called before the fallback attempt, so a crash mid-fallback still leaves the cursor correctly marked stale); a genuine non-expiry `listHistory` failure propagating as a real error rather than silently choosing a path; incremental pagination reusing the SAME `startHistoryId` (the run-scoped, not-yet-finalized cursor) across every page of one run; resuming with no `runType`, or resuming incremental with no stored cursor, both rejected via `INVALID_RESUME`; `listSyncRuns`.
- `relativity/test/emailRoutes.test.js` (extended, +1 test) — `GET /connections/:id/sync-runs` rejects an unauthenticated request with `401`, matching every other route in this file.
- Full existing suites remain green: `relativity` 610/610 (`npm test`); `aikb` unaffected by this change (2 pre-existing, unrelated environment-isolation failures, same as documented in the EM6 record).

**Deviations / implementation decisions where this document was silent, with reasons:**

1. **A resumed page mid-run that hits `HISTORY_EXPIRED` fails the whole run rather than transparently falling back a second time.** Only a FRESH call's initial probe gets the graceful "fall back within the same call" treatment (§18.4's own framing). §18.4 doesn't address a cursor expiring mid-pagination (a rare case — cursors last at least 7 days, and a resumed run typically completes within seconds of its first page), and switching strategies mid-run would corrupt the counts/token bookkeeping model already built up across prior pages of that run. A member who hits this simply clicks "Sync now" again, which then runs the fresh-call probe-and-fallback logic correctly. Flagged as a real, if narrow, scope limitation rather than silently handled.
2. **`startHistoryId` is re-read from `email_sync_state` on every resumed incremental page, not threaded through the route/`pageToken`.** Gmail's `history.list` documents `startHistoryId` as required on every call, not just the first. Re-reading it is safe specifically because the stored cursor is guaranteed unchanged until the run finally completes (`upsertSyncState` only ever writes a new cursor once `complete: true` — see the conditional-write contract above), so every page of one run reads the identical value; simpler than adding a third round-trip field to the route's request/response contract.
3. **The incremental page-size bound (`maxResults` on `history.list`) reuses the same `HISTORICAL_PAGE_SIZE = 25` constant as historical import**, rather than a separate, independently-tuned constant. §18 doesn't specify a page size for the incremental path; reusing the existing Vercel-timeout-driven bound (§17 item 3) is a reasonable default since the per-message eligibility/ingest pipeline dominates a page's runtime either way, regardless of which listing mechanism produced the candidate ids.
4. **A message's final action within one page is resolved by "last write wins" (a `Map`, iterated in the diff's own order) rather than, say, "any removal wins."** Neither §18.4 nor §24.2 specifies composition behavior for a message that changes twice in one page; last-write-wins directly reflects the message's actual current state at the moment the page was read, which is the only state that matters for deciding whether to ingest or tombstone it now.
5. **`email_sync_runs.run_type` is only ever written as `'historical'` or `'incremental'`, never the schema's third allowed value `'manual'`** (`email_sync_runs.run_type CHECK IN ('historical', 'incremental', 'manual')`, §13.1). This continues EM6's own (implicit) reading: `triggered_by_member_id`, not `run_type`, is what §15.1 says distinguishes a manual click from a future tick-triggered run — `run_type` describes the sync MECHANISM (full scan vs. diff), a orthogonal axis. Not a new deviation, just now made explicit since EM7 is the milestone that actually introduced the second mechanism value.
6. **`email_ingestion_events.ingested_document_id` remains unpopulated at ingest time for the SAME reason EM6's record already documents** (deviation 5 there) — incremental sync's `labelAdded` handling goes through the identical shared pipeline, with the identical async-enqueue limitation. Unchanged, not re-solved here.

**Testing summary:** see the Source references above for the full breakdown by file; 14 new tests in `gmailService.test.js`, 16 new tests in `emailSyncService.test.js` (42 total in that file, all EM6 tests retained), 1 new test in `emailRoutes.test.js`. `relativity` 610/610 passing across the full suite.

**Not implemented (deliberately, per EM7's own scope):** automatic-mode sync of any kind, including the tick scheduler (EM8); member offboarding reconciliation (EM9); production-quality normalization/threading/citations (EM10); attachments (EM11); Microsoft/Outlook (EM12); the `pause`/`resume` route/control (still the same EM4-flagged gap, unclaimed by any milestone); provider push notifications (Gmail Pub/Sub watch) as an alternative to polling-on-click (§18.1 — explicitly deferred, not this milestone's mechanism). **No live browser OAuth flow or real Gmail/AIKB round trip was exercised** — every claim in this record (cursor lifecycle, expiry-fallback behavior, diff-based reconciliation, last-write-wins composition) is verified through dependency-injected unit tests against a fixture mailbox and a fixture AIKB client, not a live Google Cloud OAuth Client or a real 7-day cursor-expiry window, consistent with this repo's existing no-test-database, no-real-network-call convention and the same limitation every prior EM record in this document already documents. In particular, Gmail's real `history.list` 404-on-expiry behavior is asserted only against this record's own understanding of Google's documented contract, not verified against a live account.
...
- **Status: Implemented (2026-07-24).** See the EM5 Implementation Record near the top of this document for exact source references, the seven deviations/interpretations (including the §14.1-vs-§17 query-compilation resolution and the new `getValidGmailAccessToken` refresh orchestration), and the full §28.1 fixture-driven test suite. EM6 onward remain unimplemented.
...
- **Status: Implemented (2026-07-24).** See the EM6 Implementation Record near the top of this document for exact source references, the seven deviations/interpretations (including the deliberate `automatic`-mode rejection and the "no AIKB change needed for delete" resolution), and the full DI-faked test suite. EM7 onward remain unimplemented.
...
- **Status: Implemented (2026-07-24).** See the EM7 Implementation Record near the top of this document for exact source references, the six deviations/interpretations (including the mid-run-expiry scope limitation and the shared eligibility/ingest pipeline refactor), and the full DI-faked test suite (16 new tests, 42 total in `emailSyncService.test.js`). EM8 onward remain unimplemented.
