...
**Status of this document: Proposed, Partially Implemented (EM1 only).** As of 2026-07-23, **EM1 — Multi-member schema foundation (§31)** is implemented in both repositories — see the Implementation Record immediately below. Every other milestone, **EM2 through EM13, remains unimplemented**: no OAuth flow, no Gmail/Microsoft adapter, no policy engine, no label workflow, no ingestion, no portal UI. Every claim about *current* behavior is sourced from a specific file (cited inline); everything else remains a proposal. This document does not implement the feature beyond EM1's schema — see [Constraints](#constraints-carried-into-this-plan) below.
...
Not implemented (deliberately, per EM1's own scope): any Relativity route, service, or UI; any AIKB service; Gmail/Microsoft OAuth; the managed Gmail label; policy evaluation; sync orchestration. See §31 for EM2 onward.
