...
**Status of this document: Proposed, Partially Implemented (EM1–EM3).** As of 2026-07-23, **EM1 — Multi-member schema foundation (§31)**, **EM2 — Gmail OAuth per member (§31)**, and **EM3 — Organization policy engine (§31)** are all implemented in the Relativity repository — see the Implementation Records immediately below. Every other milestone, **EM4 through EM13, remains unimplemented**: no member mailbox settings surface, no Gmail label workflow, no ingestion, no sync/scheduling, and no Microsoft/Outlook adapter. EM3 itself is policy CRUD and pure rule-evaluation logic only, exactly as scoped — no message is ever fetched, labeled, or ingested as a result of it. Every claim about *current* behavior is sourced from a specific file (cited inline); everything else remains a proposal. This document does not implement the feature beyond EM1's schema, EM2's OAuth connect flow, and EM3's policy engine — see [Constraints](#constraints-carried-into-this-plan) below.
