...
- `encryption_key_version` is stored per credential row from day one, as the seam for a future key-rotation mechanism — ~~rotation tooling itself was not built; the column is hard-coded to `1` everywhere today~~ **built (backlog M3, 2026-07-19)**: `getCurrentKeyVersion()` reads it from an env var instead of a hard-coded constant, `decryptCredential` resolves the key matching each row's own stored version, and `reencryptCredentialForConnection`/`scripts/rotateCredentialEncryptionKey.js` drive an actual rotation. See [FEATURE_BACKLOG.md](../roadmap/FEATURE_BACKLOG.md) M3.
