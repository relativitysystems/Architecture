...
│   ├── LIVE_EMAIL_LOOKUP.md           — read-only, AIKB-orchestrated live mailbox lookup as an AI agent tool (ADR-010 shape), alongside — not replacing — EMAIL_INGESTION.md. Version 1 Priority 1. EL1–EL7B implemented (tool registry, execution endpoint, real Gmail tools, bounded AIKB orchestration, portal mode selector/consent/live citations, Slack identity linking + live-email access); EL8+ (citation/audit polish) and real-account validation (EM10.5) not yet.
