...
│   ├── LIVE_EMAIL_LOOKUP.md           — read-only, AIKB-orchestrated live mailbox lookup as an AI agent tool (ADR-010 shape), alongside — not replacing — EMAIL_INGESTION.md. Version 1 Priority 1. EL1–EL6 implemented (tool registry, execution endpoint, real Gmail tools, bounded AIKB orchestration, portal mode selector/consent/live citations); EL7A+ (Slack) and real-account validation (EM10.5) not yet.
...
- The shared-first client-aware AIKB database provider described in [ADR-008](decisions/ADR-008-CLIENT-AIKB-DATABASE-ROUTING.md) is implemented (`aikb/services/aikbDatabaseProvider.js`). Remaining future work: a dedicated per-client Supabase project story (Global `client_database_assignments` routing table, credential rotation, tenant cutover/migration tooling) — see the ADR's "Future Control-Plane Model" and "Migration Strategy" sections.
