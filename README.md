...
│   ├── LIVE_EMAIL_LOOKUP.md           — read-only, AIKB-orchestrated live mailbox lookup as an AI agent tool (ADR-010 shape), alongside — not replacing — EMAIL_INGESTION.md. Version 1 Priority 1. EL1–EL5 implemented (tool registry, execution endpoint, real Gmail tools, bounded AIKB orchestration); EL6+ (portal/Slack UI) and real-account validation (EM10.5) not yet.
