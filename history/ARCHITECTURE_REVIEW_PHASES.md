...
| Slack direct messages (workspace-level authorization) | **Resolved/Implemented (backlog M13, revised)** | Any user in a connected Slack workspace may DM the bot, reusing the same client-wide `slack_collection_access` allow-list channel `@mentions` already use, plus the existing dedup, AIKB ask flow, bounded delivery retries, and terminal-failure handling — no new subsystem needed. Group DMs (`message`/`mpim`) remain explicitly unsupported. **Revised:** Slack conversations (both `@mentions` and DMs) are no longer persisted to Supabase at all — the original design created a `knowledge_chat_sessions`/`knowledge_chat_messages` row per question and merely excluded `origin: 'slack_dm'` from portal reads; `runKnowledgeQuery`'s new `persistConversation: false` mode (used for all Slack traffic) skips session/message creation entirely instead. Idempotency/dedup moved to a new content-free `knowledge_slack_request_log` table; `slack_event_log.question` was dropped. See the M13 entry in [../roadmap/FEATURE_BACKLOG.md](../roadmap/FEATURE_BACKLOG.md) for full detail. |
