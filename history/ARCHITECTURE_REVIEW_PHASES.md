...
| Slack direct messages / employee-level authorization | **Open — deferred (Milestone 7), rescoped as its own initiative (backlog M13)** | Investigated in detail; confirmed to be a multi-week effort (new OAuth scopes + workspace re-consent, new mapping table, new admin UI, new authz logic). No implementation work done or scheduled. |
