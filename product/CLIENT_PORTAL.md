...
- **Proposed**: a client-facing Knowledge Coverage surface (coverage score, missing-source recommendations) and a Knowledge Analytics v2 dashboard, replacing today's onboarding-checklist-only use of the analytics endpoint — see [KNOWLEDGE_COVERAGE.md](KNOWLEDGE_COVERAGE.md) and [KNOWLEDGE_ANALYTICS.md](KNOWLEDGE_ANALYTICS.md). Not implemented; no UI surface decided yet.
