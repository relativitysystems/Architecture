...
| **Connector completeness** | For each *connected* source, how much of what's plausibly available has actually been ingested (e.g., a Gmail connection with many messages never labeled for ingestion, or a Drive import that hasn't been refreshed since initial setup) | Per-connector sync state (`email_sync_state`, `document_import_log`, Slack collection allow-lists) |
